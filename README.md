...
실험은 ML·DL·앙상블 갈래에서 병행했습니다. 아래는 날짜별 일지가 아니라, 기록에 남은 질문과 의존 관계를 따라 정리한 진행 흐름입니다.

| 단계 | 주요 작업 | 다음 실험으로 이어진 판단 |
|---|---|---|
| 데이터 이해 | 결측, 과거 이력, 시즌별 변화, 경기 상황, TrackMan ID 체계 분석 | 이력 없음과 낮은 성공률을 구분하고 과거→미래 검증 채택 |
| 기준선 구축 | RandomForest·로지스틱 회귀, 이후 트리 모델 비교 | 복잡한 모델의 효과를 판단할 비교 기준 확보 |
| 피처·트리 개선 | CatBoost·LightGBM·XGBoost, 최근성·피처 블록·범주 처리 비교 | 단일 최고 점수와 서로 다른 오차 구조를 함께 평가 |
| 딥러닝 탐색 | FT-Transformer·RealMLP·SAINT, 의미별 피처 묶음과 잔차 학습 | 독립 성능뿐 아니라 기존 모델에 더해지는 정보량 검사 |
...
| main과 TrackMan의 ID 체계가 서로 다름 | 숫자가 같다는 가정의 직접 조인 배제, 이력 대응·물리 표현을 별도 가설로 검증 |
| 구속·릴리스·무브먼트는 손 유형과 구종군에 따라 다름 | 전체 평균 대신 조건별 물리 특성·변동성·표본 수를 후보로 검토 |

분석에서 보인 차이가 곧바로 유효한 피처가 되는 것은 아니었습니다. 예를 들어 물리 정보가 설명력이 있어 보여도,
선수 대응과 사용 시점을 보장할 수 있는지, 기존 모델 대비 추가 개선이 있는지를 각각 확인했습니다.
...
Brier는 낮을수록 좋습니다. 공식 점수는 해당 평가셋의 평균 성공률을 기준으로 Brier를 환산하므로,
**내부 검증 점수와 리더보드 점수를 같은 숫자 축의 개선 이력으로 합치지 않았습니다.**

평균 성능 외에 경기 유형, 과거 표본 수, 신규 선수, seed별 변동과 확률 보정을 함께 확인했습니다.
특히 후속 탐색에서 반복 관찰한 2024 결과는 독립적인 미관측 평가가 아닌 사후 확인으로 표시했습니다.
...
| 실험 | 시도와 관찰 | 판단 |
|---|---|---|
| ML-015 | CatBoost 244개 피처·3개 seed를 고정. 연도별 Brier가 0.243154 / 0.252100 / 0.247747로 달랐음 | 튜닝 전에 기준선을 잠그고 평균뿐 아니라 연도별 안정성 확인 |
| ENS-002 | 월별 잔차 반영량으로 2024 내부 점수가 972.45→975.02 상승했지만 실제 제출은 기준보다 하락 | 달력 기반 보정을 최종 채택하지 않음 |
| ENS-003 | TabM head 결합과 좌우타 상대 효과를 개선. `platoon_slope`의 실제 제출 기록은 1,126.33191068점 | 모델 평균뿐 아니라 특정 오차를 보완하는 구조 탐색 |
| ENS-032 | LightGBM에 XGBoost를 조건부 결합. 2024 내부 점수 783.31→810.76 | 신규 투수에 큰 보정을 주던 방향을 버리고, 이력이 충분한 Regular 구간에 반영량 집중 |
| ENS-044/045 | 고정 비율 혼합은 검증 Brier를 개선했고 패키지 일치성도 확인 | 당시 제출 기준 후보를 넘지 못한 구성은 예비안으로 보관 |
| 후반 No1·No2·JJO | 기반 모델 재구성, 잔차 학습, 경기 맥락·그래프·카운트별 개선 | 기준 모델과 실험 조건을 분명히 하며 개선·실패·제출 준비 상태를 구분 |
...
연구 과정에서는 “모델을 더 복잡하게 만들기”보다 **어느 구간에서 틀리는지 찾고, 보정의 효과가 다음 시즌에도 유지되는지 확인하는 일**이 중요했습니다.

- **로컬 개선의 전이를 확인했다.** 월별 보정처럼 내부 검증에서 좋아도 제출 결과가 나빠지는 시도가 있었습니다.
...
전체 제출 근거와 다시 진행한다면 바꿀 점은 [결과·회고](docs/results.md)에 있습니다.
